import sys
sys.path.append('/Users/rukshani/Repos/cctestbed')
...
CODEPATH = '/Users/rukshani/Repos/cctestbed'
DATAPATH_RAW = '/Users/rukshani/Documents/DATASET/WEEK_22/loss/FULL/COMBI/data-raw'
DATAPATH_PROCESSED = '/Users/rukshani/Documents/DATASET/WEEK_22/loss/FULL/COMBI/data-processed'
# CODEPATH = '/opt/cctestbed'
# DATAPATH_RAW = '/tmp/data-raw'
# DATAPATH_PROCESSED = '/tmp/data-processed'
BYTES_TO_BITS = 8
BITS_TO_MEGABITS = 1.0 / 1000000.0
REMOTE_IP_ADDR = '128.105.145.77'
REMOTE_USERNAME = 'rukshani'
...
class ExperimentAnalyzer:
    def __init__(self, experiment, load_queue=False):
        ...
        self._flow_receiver_ports = None
        self._flow_names = None
        self._df_pcap_tcp = None
        self._http_pcap = None 
        self._server_tcpdump_youtube = None
        self._server_tcpdump_tcp = None
        self._server_tcpdump_http = None 
        self._client_tcpdump_http = None 
    ...
    @property
    def flow_receiver_ports(self):
        if self._flow_receiver_ports is None:
            self._flow_receiver_ports = {flow.server_port:flow.ccalg
                                        for flow in self.experiment.flows}
        return self._flow_receiver_ports

    #Use only for solo
    @property
    def get_ccalgs(self):
        return {'cca':flow.ccalg for flow in self.experiment.flows}
    ...
    def http_pcap(self):
        print("http pcap")
        http_tarpath = self.experiment.logs['http_log']
        http_processed_localpath = os.path.join(DATAPATH_PROCESSED, http_tarpath)[:-len('.pcap')] + '.csv'
        with untarfile_extract(self.experiment.tarfile_localpath, http_tarpath) as http_localpath:
            print(http_localpath)
            # cmd = "tshark -r {} -V -Y http.request -T fields -e frame.time_relative -e http.host -e ip.src -e ip.dst -e http.request.full_uri  -e tcp.srcport -e tcp.dstport -e http.header.Range >> {}".format(
            #     http_localpath, http_processed_localpath)
            cmd = "tshark -r {} -V -Y http.request -T fields -e frame.time_relative -e http.host -e ip.src -e ip.dst -e http.request.full_uri  -e tcp.srcport -e tcp.dstport >> {}".format(
                http_localpath, http_processed_localpath)
            # print(cmd)
            proc = subprocess.run(cmd, shell=True, stderr=subprocess.PIPE)
            if proc.returncode != 0:
    ...
    @property
    def server_tcpdump_tcp(self):
        print("server tcpdump")
        http_tarpath = self.experiment.logs['server_tcpdump_log']
        http_processed_localpath = os.path.join(DATAPATH_PROCESSED, http_tarpath)[:-len('.pcap')] + '.csv'
        with untarfile_extract(self.experiment.tarfile_localpath, http_tarpath) as http_localpath:
            print(http_localpath)
            cmd = "tshark -r {} -T fields -e frame.time_relative -e http.host -e ip.src -e ip.dst -e http.request.full_uri  -e tcp.srcport -e tcp.dstport >> {}".format(
                http_localpath, http_processed_localpath)
            # print(cmd)
            proc = subprocess.run(cmd, shell=True, stderr=subprocess.PIPE)
            if proc.returncode != 0:
    ...
    @property
    def server_tcpdump_http(self):
        print("server tcpdump")
        http_tarpath = self.experiment.logs['server_tcpdump_log']
        http_processed_localpath = os.path.join(DATAPATH_PROCESSED, http_tarpath)[:-len('.pcap')] + '.csv'
        with untarfile_extract(self.experiment.tarfile_localpath, http_tarpath) as http_localpath:
            print(http_localpath)
            cmd = "tshark -r {} -Y http.request -T fields -e frame.time_relative -e http.host -e ip.src -e ip.dst -e http.request.full_uri  -e tcp.srcport -e tcp.dstport >> {}".format(
                http_localpath, http_processed_localpath)
            # print(cmd)
            proc = subprocess.run(cmd, shell=True, stderr=subprocess.PIPE)
            if proc.returncode != 0:
    ...
    @property
    def client_tcpdump_http(self):
        print("server tcpdump")
        http_tarpath = self.experiment.logs['client_tcpdump_log']
        http_processed_localpath = os.path.join(DATAPATH_PROCESSED, http_tarpath)[:-len('.pcap')] + '.csv'
        with untarfile_extract(self.experiment.tarfile_localpath, http_tarpath) as http_localpath:
            print(http_localpath)
            cmd = "tshark -r {} -Y http.request -T fields -e frame.time_relative -e http.host -e ip.src -e ip.dst -e http.request.full_uri  -e tcp.srcport -e tcp.dstport >> {}".format(
    ...
    #Only for youtube pcap analysis
    @property
    def server_tcpdump_youtube(self):
        print("server tcpdump")
        http_tarpath = self.experiment.logs['server_tcpdump_log']
        ssl_keylog_path = self.experiment.logs['ssl_key_log']
        http_processed_localpath = os.path.join(DATAPATH_PROCESSED, http_tarpath)[:-len('.pcap')] + '.csv'
        with untarfile_extract(self.experiment.tarfile_localpath, http_tarpath) as http_localpath:
            print(http_localpath)
            with untarfile_extract(self.experiment.tarfile_localpath, ssl_keylog_path) as ssl_localpath:
                cmd = "tshark -r {} -Y http.request -T fields -e frame.time_relative -e http.host -e ip.src -e ip.dst -e http.request.full_uri -e tcp.srcport -e tcp.dstport -o ssl.keylog_file:{} >> {}".format(
                    http_localpath, ssl_localpath, http_processed_localpath)
                proc = subprocess.run(cmd, shell=True, stderr=subprocess.PIPE)
                if proc.returncode != 0:
                    raise RuntimeError('Encountered error running cmd {}\n{}'.format(cmd, proc.stderr.decode('utf-8')))
    
    @property
    def df_pcap_tcp(self):
        print("cp tcpsump")
