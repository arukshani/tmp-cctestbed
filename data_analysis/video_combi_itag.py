...
def process_http_pcap(experiment_analyzer, app_type, port):
    filepath = ex.DATAPATH_PROCESSED + "/server-tcpdump-" + app_type + ".csv"
    df = pd.read_csv(filepath ,sep='\t', 
                  names=["Time", "dstip_dstport", "srcip", "dstip", "request_uri", "scrport", "dstport"])
    rows_for_port = df.loc[df['dstport'] == port]
    # print(rows_for_port)
    rows_for_port['path'] = rows_for_port.request_uri.apply(extract_path)
    # print(rows_for_port['path'])
    rows_for_port['mime'] = rows_for_port.path.apply(extract_mime_DASH)
    rows_for_port['itag_str'] = rows_for_port.path.apply(extract_itag_DASH)
    rows_for_port['bitrate'] = rows_for_port['itag_str'].apply(map_bitrate)
    rows_for_port['bitrate_mbps'] = rows_for_port['bitrate'].apply(convert_bitrate_mbps)
    return rows_for_port 
...
def avg_bitrate_youtube_per_experiment(all_dataframes):
    for key, value in all_dataframes.items():
        ...
        if not bitrates.empty:
            ...
            new_df.to_csv(ex.DATAPATH_PROCESSED +'/harm/combi-results/combi_webVideo_bitrate.csv', header=False, index=False, mode = 'a')
...
def avg_bitrate_video_per_experiment(all_dataframes):
    for key, value in all_dataframes.items():
        ...
        data = [[mean, key, metric]]
        new_df = pd.DataFrame(data)
        new_df.to_csv(ex.DATAPATH_PROCESSED +'/harm/combi-results/combi_localVideo_bitrate.csv', header=False, index=False, mode = 'a')

def final_avg_localVideo():
    all_bitrate_data = pd.read_csv(ex.DATAPATH_PROCESSED + '/harm/combi-results/combi_localVideo_bitrate.csv' ,sep=',', 
    ...
    avg_output.to_csv(ex.DATAPATH_PROCESSED +'/harm/combi-results/avg-bitrate-' + 'localVideo' + '.csv', header=False, index=True)

def final_avg_youtube():
    all_bitrate_data = pd.read_csv(ex.DATAPATH_PROCESSED + '/harm/combi-results/combi_webVideo_bitrate.csv' ,sep=',', 
                  names=["bitrate", "filename", "metric"])
    ...
    avg_output = all_bitrate_data.groupby('group')['bitrate'].mean()
    # print(avg_output)
    avg_output.to_csv(ex.DATAPATH_PROCESSED +'/harm/combi-results/avg-bitrate-' + 'webVideo' + '.csv', header=False, index=True)


# all_youtube_dataframes = {}
# all_dash_dataframes = {}
# # all_analysers = cmn.get_experiment_analysers("50bw-20rtt-256q-iperf-webVideotest-1-1-cubic-none--60s-20200813T125228")
# all_analysers = cmn.get_all_experiment_analysers()
# for analyser in all_analysers:
#     print("~~~~~~~~",analyser, "~~~~~~")
#     if "iperf-localVideo" in analyser:
#         # extract_tcpdump_http(all_analysers[analyser], analyser)
#         dash_df = process_http_pcap(all_analysers[analyser], analyser, 5203)
#         all_dash_dataframes[analyser] = dash_df
#     elif "iperf-webVideo" in  analyser:
#         # ~~~~~~~~~~~~~~~YOUTUBE_ANALYSIS~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# fig.update_layout(
#             title="iperf-localDASH & iperf-YouTube - Video quality level change over time",
#             xaxis_title="<b>Time (s)</b>",
#             yaxis_title="<b>Bitrate (Mbps)</b>",
#         )

# filename_plot = ex.DATAPATH_PROCESSED + "/harm/combi_"+ "video_itags_3G"+ ".html"
...
# fig1.update_layout(
#             title="iperf-localDASH & iperf-YouTube - Audio quality level change over time",
#             xaxis_title="<b>Time (s)</b>",
#             yaxis_title="<b>itag </b>",
#         )

# filename_plot1 = ex.DATAPATH_PROCESSED + "/harm/combi_"+ "audio_itags_3G"+ ".html"
