...
AWS_REGIONS = [
    #'eu-north-1', # New AWS region, no image for this one
    'ap-south-1',
    'eu-west-3',
    'eu-west-2',
    'eu-west-1',
...
#AMI_OWNER_USER_ID='524904575043' # This is the ID for the user that made all the disk images

class AWSHost:
    """Represent each AWS Host in a specific AWS region"""

    def __init__(self, region_name):
        """
        Args:
            region_name (str): name of the region. should be one of the available AWS regions
        """
        if region_name not in AWS_REGIONS:
            raise ValueError('{} is not an available AWS region. Expected one of: {}.'.format(region_name, AWS_REGIONS))
        self.region_name = region_name
        self._ec2 = boto3.client('ec2', region_name=region_name)

        # get disk id for this region
        disk_image_filter = [{'Name':'name', 'Values':[self.region_name]}, {'Name':'is-public', 'Values':['false']}]
        response = self._ec2.describe_images(Filters=disk_image_filter)
        print(response)
        # assert len(response['Images']) == 1
        #self.disk_image_id = response['Images'][0]['ImageId']
        self.disk_image_id = 'ami-03e6d1feebffa43d6'

    def get_instance(self):
        """Return running instance in this AWS region. Will return None if there is no running instance."""
        running_filter = [{'Name': 'instance-state-name', 'Values': ['running']},{'Name': 'image-id', 'Values': [self.disk_image_id]}]
    ...
    def start_instance(self):
        ...
        if instance is None:
            instance =  self._region_start_instance()
        try:
            yield instance
        finally:
            instance.terminate()

    def _region_start_instance(self):
        # find an availabilty zone
        available_zones = [zone['ZoneName'] for zone in self._ec2.describe_availability_zones(Filters=[{'Name':'state', 'Values':['available']}])['AvailabilityZones']]
        print(available_zones)
        # create 1 ubuntu t2.micro instance
        instance = boto3.resource('ec2', region_name=self.region_name).create_instances(
            ImageId=self.disk_image_id,
            InstanceType='t2.micro',
            Placement={
                'AvailabilityZone': available_zones[0]},
            KeyName='{}_cloudlab'.format(os.environ['USER']),
            NetworkInterfaces=[
        ...
        ssh_allow_rule = {'FromPort': 22,
                        ...
                        'UserIdGroupPairs': []}
        try:
            response = self._ec2.authorize_security_group_ingress(GroupName='default',
...
def get_key_name(ec2):
    ...
    if len(key_pairs) == 0:
        ...
        for key_pair in key_pairs:
            if key_pair['KeyName'].startswith('rukshani'):
...
def _region_start_instance(ec2, image_id=None):
    ...
    # key_name = "rukshani_cloudlab"
    # create 1 ubuntu t2.micro instance
    print(region_name, "image>>", image_id, "available_zone>>", available_zone, "key_name>>", key_name)
...
@contextmanager
def add_nat_rule(instance, nat_ip='128.2.208.128', nat_username='rukshani',
                 nat_key_filename='/users/rukshani/.ssh/id_rsa'):
...
def _add_nat_rule(instance, nat_ip='128.2.208.128', nat_username='rukshani',
                 nat_key_filename='/users/rukshani/.ssh/id_rsa'):
    with command.get_ssh_client(ip_addr=nat_ip, username=nat_username,
                                key_filename=nat_key_filename) as ssh_client:
    ...
    for region in regions:
        ...
        if (instance is None) or (force_create_instance):
            ...
            if image is None:
                ...
                image_id = 'ami-03e6d1feebffa43d6' #CLARIFY
                print("image id:", image_id)
                print("key name", get_key_name(ec2_region))
            logging.info('Creating instance for region {}'.format(region))
            instance = _region_start_instance(ec2_region, image_id)
            try:
                instance.wait_until_running()
                instance.load()
                if image is None: #CLARIFY:image doesn't have cctestbed setup and kernel module installed hence commenting
        ...
        exit_status, stdout = install_iperf3(ec2_region, instance, ec2_username='ubuntu')
...
"""
        ...
        },
        {
            ...
            "Resource": "arn:aws:iam::${aws:userid}:root"
        },
        {
