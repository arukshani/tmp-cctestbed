...
def run_experiment(website, url, btlbw=10, queue_size=128, rtt=35, force=False, resolution=(1280,720)):
    ...
    with ExitStack() as stack:
        # add DNAT rule
        stack.enter_context(add_dnat_rule(exp, url_ip, video_url_ip))
        # add route to URL
        stack.enter_context(add_route(exp, url_ip, video_url_ip))
        # add dns entry
        ...
        with cctestbed.get_ssh_client(exp.server.ip_wan,
            ...
            print("video url :", url)
            print("resolution width and height :", resolution[0][0], resolution[0][1])
            #windowSize = "1280,720"
            start_flow_cmd = 'timeout {}s google-chrome --headless --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --window-size={},{} --start-maximized {}'.format(120+5, resolution[0][0], resolution[0][1], url)
...
def get_website_ip(url):
    ...
    # ip_addrs = cctestbed.run_local_command(
    #     "nslookup {} | awk '/^Address: / {{ print $2 ; exit }}'".format("googlevideo.com"), shell=True)
    ip_addr = ip_addrs.split('\n')[0]
    print("ipaddress of website {}", ip_addr)
    if ip_addr.strip() == '':
...
def main(websites, ntwrk_conditions=None, force=False, resolution=None):
    completed_experiment_procs = []
    ...
    if ntwrk_conditions is None:
        ntwrk_conditions = [(1,35), (5,85), (5,130), (5,275),
                            (3,35), (10,85), (10,130), (10,275),
                            (5,35), (15,85), (15,130), (15,275)]

        #  ntwrk_conditions = [(0.5,35), (0.5,85), (0.5,130), (0.5,275),
        #                     (1,35), (1,85), (1,130), (1,275),
        #                     (1.5,35), (1.5,85), (1.5,130), (1.5,275), (1, 550)]
        
    for website, url in websites:
        try:
            num_completed_experiments = 1
            too_small_rtt = 0
            for btlbw, rtt_str in ntwrk_conditions:
                btlbw = float(btlbw)
                rtt = int(rtt_str)
                queue_size = QUEUE_SIZE_TABLE[rtt][btlbw]                    
                print('Running experiment {}/{} website={}, btlbw={}, queue_size={}, rtt={}, resolution={}.'.format(
                    num_completed_experiments,len(websites)*len(ntwrk_conditions), website,btlbw,queue_size,rtt, resolution))
                num_completed_experiments += 1
                if rtt <= too_small_rtt:
                    print('Skipping experiment RTT too small')
                    continue
                (proc, exp_name) = run_experiment(website, url, btlbw, queue_size, rtt, force=force, resolution=resolution)
...
def parse_args():
    ...
    parser.add_argument(
        '--network, -n', nargs=2, action='append', metavar=('BTLBW','RTT'), dest='ntwrk_conditions', default=None, 
        help='Network conditions for download from website.')
    parser.add_argument(
        '--resolution, -r', nargs=2, action='append', metavar=('WIDTH','HEIGHT'), dest='resolution', default=None, type=int,
...
if __name__ == '__main__':
    ...
    args = parse_args()
    main(args.websites, ntwrk_conditions=args.ntwrk_conditions, force=args.force, resolution=args.resolution)
