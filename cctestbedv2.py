...
Flow = namedtuple('Flow', ['ccalg', 'start_time', 'end_time', 'rtt',
                           ...
                           'server_log', 'kind', 'client', 'webInfo'])
...
class Experiment:
    def __init__(self, name, btlbw, queue_size,
        ...
        self.logs = {
            ...
            'ssl_key_log': '/tmp/sslkeylog-{}-{}.log'.format(self.name, self.exp_time)
            }
    ...
    def _copy_ssl_key_log_file(self):
        
        ssl_file_path = '/users/rukshani/SSLkeylogs/sslkeylog-{}-{}.log'.format(self.name, self.exp_time)
        cmd = 'scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i {} {}@{}:{} /tmp/'.format(self.server.key_filename, self.server.username,
                                                      self.server.ip_wan,
                                                      os.path.join('/tmp',ssl_file_path))
        
        # cmd = 'scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {}@{}:{} /tmp/'.format(self.server.username, self.server.ip_wan,
        #                                         os.path.join('/tmp', ssl_file_path))
        print('Copying remotepath {} to localpath {}'.format(
                    ssl_file_path, '/tmp'))
        # cmd = 'scp -r {}@{}:{} {}'.format(self.server.username, self.server.ip_wan,
        #                                 ssl_file_path, '/tmp')
    ...
    def _run_tcpprobe(self, stack):
        ...
        print("client_ip_wan:username", self.client.ip_wan, self.client.username)
        with get_ssh_client(self.client.ip_wan,
                            username=self.client.username,
                            key_filename=self.client.key_filename) as ssh_client:
    ...
    def _run_bess(self, ping_source='client', skip_ping=False, bess_config_name='active-middlebox-pmd'):
        ...
        try:
            ...
            if ping_source == 'client':
                ...
                print("Ping source client>>", ping_source_ip, ping_dest_ip, ping_ssh_ip, ping_ssh_username, ping_ssh_key_filename)
            ...
            if ping_source == 'server':
                ...
                cmd = ('ping -c 10 -I {} {} '
                                      '| tail -1 '
                                      '| awk "{{print $4}}" ').format(ping_source_ip, ping_dest_ip)
            with get_ssh_client(ping_ssh_ip,
                ...
                try:
                    ...
                    if ping_source == 'server':
                        ...
                        print("avg_rtt>>", line, stderr)
                    else:
                        print("ping output>>", line, stderr)
    ...
    def _start_video_flow(self, flow, stack):
        ...
        web_download_cmd = 'timeout {}s google-chrome --disable-gpu --headless --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --window-size={},{} --start-maximized "http://{}:1234/"'.format(flow.end_time, 1280, 720, self.client.ip_lan)
