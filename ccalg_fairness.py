...
def run_experiment_1vmany(website, url, competing_ccalg, num_competing,
    ...
    for x in range(num_competing):
        ...
        flows.append(cctestbed.Flow(ccalg=competing_ccalg,
                                    ...
                                    client=HOST_CLIENT, webInfo=None))
...
def run_youtube_experiment(website, url, 
                          btlbw=10, queue_size=128, rtt=35, duration=60,
                          chrome=False):
    print("chrome", chrome)
    experiment_name = '{}bw-{}rtt-{}q-{}-{}s'.format(btlbw, rtt,
                                                      queue_size, website,duration)
    logging.info('Creating experiment for website: {}'.format(website))
    url_ip = get_website_ip(url)
    logging.info('Got website IP: {}'.format(url_ip))
    website_rtt = int(float(get_nping_rtt(url_ip)))
    ...
    print("server ip wan", exp.server.ip_wan)
    ...
    with ExitStack() as stack:
        # add DNAT rule
        #stack.enter_context(add_dnat_rule(exp, url_ip, chrome=chrome))
        stack.enter_context(add_dnat_rule_video(exp, url_ip, video_url_ip))
        # add route to URL
        #stack.enter_context(add_route(exp, url_ip, chrome=chrome))
        stack.enter_context(add_route_video(exp, url_ip, video_url_ip))
        # add dns entry
        stack.enter_context(add_dns_rule(exp, website, url_ip))
        stack.enter_context(add_dns_rule(exp, video_server_host, video_url_ip))
        exp._run_tcpdump('server', stack)
        # run the flow
        # turns out there is a bug when using subprocess and Popen in Python 3.5
        # so skip ping needs to be true
        ...
        with cctestbed.get_ssh_client(exp.server.ip_wan,
            ...
            if filename.strip() == '':
                logging.warning('Could not get filename from URL')
            if chrome:
                start_flow_cmd = 'timeout {}s google-chrome --headless --ssl-key-log-file=/users/rukshani/SSLkeylogs/sslkeylog.log --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --window-size={},{} --start-maximized {}'.format(duration+5, 1920, 1080, url)
                #start_flow_cmd = 'timeout {}s google-chrome --headless --ssl-key-log-file=/users/rukshani/SSLkeylogs/sslkeylog.log --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --start-maximized {}'.format(duration+5, url)
                #start_flow_cmd = 'timeout {}s google-chrome --headless --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --window-size={},{} {}'.format(duration+5, 1280, 720, url)
            else:
                start_flow_cmd = 'timeout {}s wget --no-check-certificate --no-cache --delete-after --connect-timeout=10 --tries=3 --bind-address {}  -P /tmp/ {} || rm -f /tmp/{}.tmp*'.format(duration+5, exp.server.ip_lan, url, filename)
            print("start_flow_cmd>>",start_flow_cmd)
            # won't return until flow is done
            flow_start_time = time.time()
            _, stdout, _ = cctestbed.exec_command(ssh_client, exp.server.ip_wan, 'export SSLKEYLOGFILE=/users/rukshani/SSLkeylogs/sslkeylog.log') #This is to decrypt traffic
...
def run_iperf_youtube_experiment(website, url, competing_ccalg, num_competing,
                          btlbw=10, queue_size=128, rtt=35, duration=60,
                          chrome=False):
    print("chrome", chrome)
    experiment_name = '{}bw-{}rtt-{}q-{}-{}{}-{}s'.format(btlbw, rtt,
    ...
    server_nat_ip = HOST_CLIENT.ip_wan #'128.104.222.182'  taro
    print('server_nat_ip>',server_nat_ip)
    server_port = 5201
    client_port = 5555
    ...
    with ExitStack() as stack:
        ...
        with cctestbed.get_ssh_client(exp.server.ip_wan,
            ...
            if filename.strip() == '':
                logging.warning('Could not get filename from URL')
            if chrome:
                # start_flow_cmd = 'timeout {}s google-chrome --headless --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --window-size={},{} --start-maximized {}'.format(duration+5, 1280, 720, url)
                start_flow_cmd = 'timeout {}s google-chrome --headless --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --start-maximized {}'.format(duration+5, url)
                #start_flow_cmd = 'timeout {}s google-chrome --headless --remote-debugging-port=9222 --autoplay-policy=no-user-gesture-required --window-size={},{} {}'.format(duration+5, 1280, 720, url)
            else:
                start_flow_cmd = 'timeout {}s wget --no-check-certificate --no-cache --delete-after --connect-timeout=10 --tries=3 --bind-address {}  -P /tmp/ {} || rm -f /tmp/{}.tmp*'.format(duration+5, exp.server.ip_lan, url, filename)
            print("start_flow_cmd>>",start_flow_cmd)
...
def run_youtube_iperf_experiment(website, url, competing_ccalg, num_competing,
                          btlbw=10, queue_size=128, rtt=35, duration=60,
                          chrome=False):
    print("chrome", chrome)
    experiment_name = '{}bw-{}rtt-{}q-{}-{}{}-{}s'.format(btlbw, rtt,
...
def main(tests, websites,
    ...
    if ntwrk_conditions is None:
        ntwrk_conditions = [(5,35), (5,85), (5,130), (5,275),
                            ...
                            (15,35), (15,85), (15,130), (15,275), (50,256)]
    ...
    for params in exp_params:
        try:
            ...
            if test == 'iperf':
                ...
                (proc, exp_name)   = run_youtube_experiment(website,
                                                           url,
                                                           btlbw,
                                                           queue_size,
                                                           rtt,
                                                           duration,
                                                           chrome)
            elif test == 'iperf-youtube':
                (proc, exp_name)   = run_iperf_youtube_experiment(website,
                                                           url,
                                                           competing_ccalg,
                                                           num_competing,
                                                           btlbw,
                                                           ...
                                                           duration,
                                                           chrome)
            elif test == 'youtube-iperf':
                (proc, exp_name)   = run_youtube_iperf_experiment(website,
                                                           url,
                                                           competing_ccalg,
                                                           num_competing,
                                                           btlbw,
...
def parse_args():
    ...
    parser.add_argument('--test, -t', choices=[
        'apache','iperf','video','apache-website','iperf-website', 'youtube', 'iperf-youtube', 'youtube-iperf',
