...
def parse_args():
    ...
    parser.add_argument(
        '--website, -w', nargs="*", action='append', metavar=('WEBSITE', 'FILE_URL'), dest='websites',
        help='Url of file to download from website. File should be sufficently big to enable classification.')
    parser.add_argument(
        '--network, -n', nargs=3, action='append', metavar=('BTLBW','RTT', 'QUEUE_SIZE'), dest='ntwrk_conditions', type=int,
        help='Network conditions for download from website.')
    parser.add_argument(
        '--num_competing','-c', type=int, action='append', dest='nums_competing') #for single experiments just include one value, for combinations a list [1, 2]
    parser.add_argument(
        '--competing_ccalg','-a', choices=['cubic','bbr', 'reno', 'none', 'vegas', 'yeah', 'westwood'], dest='competing_ccalgs', action='append')
    parser.add_argument(
