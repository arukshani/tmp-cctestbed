...
#This script is deprecated. To run solo experiments use solo.py and for combinatory experiments combinatory.py
...
def parse_args():
    ...
    parser.add_argument('--test, -t', nargs='*', help='iperf, localWebsite, localVideo, webVideo, website and any combinations', dest='tests', default=['website'])
    parser.add_argument(
        '--website, -w', nargs=2, action='append', metavar=('WEBSITE', 'FILE_URL'), dest='websites',
        help='Url of file to download from website. File should be sufficently big to enable classification.', default=[('www.cypress.io', 'https://download.cypress.io/desktop')])
    parser.add_argument(
    ...
    parser.add_argument(
        '--num_competing','-c', type=int, action='append', dest='nums_competing', default=1) #for single experiments just include one value, for combinations a list [1, 2]
    parser.add_argument(
        '--competing_ccalg','-a', choices=['cubic','bbr', 'reno', 'none'], dest='competing_ccalgs', action='append', default=['reno', 'reno'])
    parser.add_argument(
